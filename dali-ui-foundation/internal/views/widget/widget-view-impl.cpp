...
Dali::WidgetEngine gDefaultEngine;
...
{
  gDefaultEngine = std::move(engine);
}
...
Dali::Ui::WidgetView WidgetView::New(const std::string& widgetId, const std::string& contentInfo, int width, int height, float updatePeriod)
{
  ...
  return New(gDefaultEngine.CreateWidget(widgetId, contentInfo, width, height, updatePeriod), width, height);
}
...
void WidgetView::InitializeLayout()
{
  ...
  mWidgetImage = Dali::Ui::ImageView::New();
  ConfigureImageView(mWidgetImage, widgetSize);
  Constraint widgetCornerConstraint = Constraint::New<Vector4>(mWidgetImage, Dali::Ui::View::Property::CORNER_RADIUS, EqualToConstraint());
  widgetCornerConstraint.AddSource(Source(self, Dali::Ui::View::Property::CORNER_RADIUS));
  widgetCornerConstraint.Apply();
  self.Add(mWidgetImage);
...
void WidgetView::UpdateWidgetImage(Dali::NativeImagePtr image)
{
  ...
  auto imageUrl = Dali::Ui::ImageUrlUtils::GenerateUrl(mImageSource, true);
  // Keep the ImageUrl attachment alive until ImageView replaces this resource.
  // Passing only its string URL would remove the external texture registration
  // when this local ImageUrl is destroyed.
  mWidgetImage.SetResourceUrl(imageUrl);
  ApplyEffect();
  mWidgetImage.SetProperty(Actor::Property::VISIBLE, true);
...
void WidgetView::HandleEngineEvent(Dali::WidgetEngineInstancePlugin::EventType event)
{
  ...
  switch(event)
  {
    ...
    case Dali::WidgetEngineInstancePlugin::EventType::FAULTED:
      mWidgetImage.SetProperty(Actor::Property::VISIBLE, false);
      ShowRetryState(true);
      NotifyAccessibilityChildrenChanged();
...
{
  if(mEffectPropertyMap.Empty() || !mWidgetImage)
  {
    return;
  }

  auto visual = ViewDataImpl::Get(Dali::Ui::GetImpl(mWidgetImage)).GetVisual(Dali::Ui::ImageView::Property::IMAGE);
  if(visual)
  {
    Property::Map properties;
    properties.Insert(Dali::Ui::VisualBasePropertyIndex::SHADER, mEffectPropertyMap);
