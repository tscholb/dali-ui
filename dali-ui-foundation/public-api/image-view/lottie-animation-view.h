...
public:
  ...
  struct DynamicPropertyInfo
  {
    ...
    int32_t       property; ///< The VectorProperty::Type to animate
...
public: // Frame Range
   ...
   *
   * If only @p marker1 is given, the animation plays the range of that marker.
   * If both are given, the animation plays from the start of @p marker1 to the
   * end of @p marker2.
   *
   * @param[in] marker1 Name of the first marker
   * @param[in] marker2 Name of the second marker (optional)
   * @return Reference to this for fluent chaining
   */
  LottieAnimationView& SetMinMaxFrameByMarker(const Dali::String& marker1, const Dali::String& marker2 = "");
...
public: // Advanced
  ...
  /**
   * @brief Flushes pending animation property changes to the render thread.
   *
   * Call this after a series of property changes to ensure they are applied
   * together in the next rendered frame.
   *
   * @return Reference to this for fluent chaining
   */
  LottieAnimationView& Flush();
