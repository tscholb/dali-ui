...
/**
 * @brief Verifies LottieAnimationView SetDepthIndex.
 *
 * SetDepthIndex controls the rendering order of visuals within the same
 * Control. This view has two visuals: the Lottie visual and the
 * background visual (red fill).  A meaningful visual change only happens
 * when the animation is moved *behind* the background, so two values are
 * enough to verify the API:
 *
 *   [Depth: -1] -> Lottie drawn behind the background fill; only red shows.
 *   [Depth:  0] -> default order; Lottie sits above the background.
 *
 * Any non-negative value (0, 1, 5, ...) produces an identical result
 * because the only other visual (background) sits below, so there is
 * nothing for the Lottie to be "above" more than it already is. This is
 * expected behaviour, not a bug.
 */
class TcLottieDepthIndex : public ManualTest::TestCase, public ConnectionTracker
{
public:
  Dali::String GetName() const override
  {
    return "Lottie: DepthIndex";
  }

  Dali::String GetDescription() const override
  {
    return "Depth < 0: Lottie hidden behind background fill; Depth >= 0: identical (Lottie on top)";
  }
  ...
  void OnEnter(View contentArea) override
  {
    ...
    mLabel = MakeStatusLabel(
      ...
      "This view has 2 visuals: Lottie + background.\n"
      "-1 hides Lottie behind background; 0+ all look identical.");
...
REGISTER_MANUAL_TEST(TcLottieDepthIndex)
